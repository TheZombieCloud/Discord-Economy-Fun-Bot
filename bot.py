...
TOKEN = 'Mzk0MjkxNjYzMDQ1OTE4NzY0.DosYHA.OWLyxrFPhDVfpO9xvHloXhbAek4'
...
async def on_message(message):
    ...
    if message.content.startswith('YU:'):
        if (len(message.content)==11 and message.content[3:11] == "register"):
            ...
            if balance[1:len(balance)-2]=="":
                embed = discord.Embed(color = 0x45F4E9)
                embed.add_field(name = "Balance", value = 'Register an account using \"YU:register\"')
                await client.send_message(message.channel, embed = embed)
            else:
                embed = discord.Embed(color = 0x45F4E9)
            ...
            if (balance>=bet):
                ...
                message2 = await client.send_message(message.channel, embed = embed)
                time.sleep(3)
                await client.delete_message(message2)
                if (random.randint(0,1)):
                    embed2 = discord.Embed(color = 0x45F4E9)
                    data_edit(message.author.id, 2, bet, balance)
                    embed2.add_field(name = "Congrats", value = "You won " + str(bet*2) + " bits.")
                    await client.send_message(message.channel, embed = embed2)
                else:
                    embed2 = discord.Embed(color = 0x45F4E9)
                    data_edit(message.author.id, 1, -bet, balance)
            ...
            str2 = data_editdaily(message.author.id)
            embed = discord.Embed(color = 0x45F4E9)
            if (str2=="1"):
            ...
            if (len(mentions)>1):
                ...
                if bet>otherbal:
                    embed = discord.Embed(color = 0x45F4E9)
                    embed.add_field(name = "Sorry", value = "You can't steal more than the person has lmao")
                    await client.send_message(message.channel, embed = embed)
                elif bet>balance:
                    embed = discord.Embed(color = 0x45F4E9)
            ...
            embed.add_field(name = "YU:register", value = "Register an account and earn 10000 bits instantly.")
            embed.add_field(name = "YU:balance", value = "Check your balance.")
            embed.add_field(name = "YU:coin <bet>", value = "Flip a Coin. Replace <bet> with your bet. Win = 2x. Lose = 0x")
            embed.add_field(name = "YU:steal <user> <bet>", value = "Steal from someone. Replace <user> with the user you want to steal from. Replace <bet> with your bet. 1/3 Success.")
            await client.send_message(message.channel, embed = embed)
